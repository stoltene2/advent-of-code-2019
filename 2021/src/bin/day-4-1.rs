...
fn main() {
    ...
    // Make this a test case for matching on columns
    let calls = vec![14, 10, 18, 22, 2];

    for i in calls {
        board.mark(i);
    }

    // todo: need to set cols
    ...
    for i in calls {
        ...
        if let Some(b) = winner {
            ...
            let unmarked_sum = b.unmarked_sum();
            //println!("Result: {}", unmarked_sum * i);

            // For part 2, remove all but the last one
            //break;
            ...
            if boards.len() == 1 {
                // 7700 is too high
                println!("Last winning board: {}", unmarked_sum * i);
            }
        }
    }
}
